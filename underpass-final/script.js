(() => {
  ...
  function pulseGlow(className, duration) {
    ...
    if (!el) return;
    el.classList.remove('pulse-warm', 'pulse-dawn');
  ...
  function spawnZzz() {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      el.className = 'zzz';
      el.textContent = 'z';
      const left = 49 + Math.random() * 4;   // clustered near the pillow end of the bed
      const top = 60 + Math.random() * 3;
      const delay = i * 350;
      const size = 12 + i * 3;               // each "z" a little bigger than the last
      el.style.cssText = [
        `left:${left.toFixed(1)}%`,
        `top:${top.toFixed(1)}%`,
        `font-size:${size}px`,
  ...
  function playFireMilestone() {
    playFx('fire');
    const el = FX.fire.el;
    if (el) {
      el.classList.remove('fx-milestone-pulse');
      void el.offsetWidth;
      el.classList.add('fx-milestone-pulse');
    }
    pulseGlow('pulse-warm', 900);
  }

  function playBedMilestone() {
    playFx('bed');
    spawnZzz();
  }

  function playPuddleMilestone() {
    // a second ripple right behind the first, so it reads as one bigger
    // splash rather than just a repeat
    playFx('puddle');
    const fx = FX.puddle;
    const totalMs = fx.frames * fx.delay + 150;
    trackTimer(fx.el, setTimeout(() => playFx('puddle'), totalMs + 120));
  }
  ...
  const HOTSPOT_MILESTONES = {
    fire:    { every: 7, run: playFireMilestone,    caption: '장작을 더 던진 듯, 불꽃이 크게 솟아오릅니다!' },
    bed:     { every: 8, run: playBedMilestone,     caption: '이불 속에서 새근새근, 잠이 깊어집니다.' },
    puddle:  { every: 6, run: playPuddleMilestone,  caption: '물결이 두 번, 크게 퍼져나갑니다.' },
    lantern: { every: 9, run: playLanternMilestone, caption: '랜턴 불빛이 잠시 신비로운 빛깔로 물듭니다.' },
    tarp:    { every: 5, run: playTarpMilestone,    caption: '센 바람이 불어와 천막 자락과 낙엽이 함께 흩날립니다.' },
  };
  const hotspotClickCounts = { fire: 0, bed: 0, puddle: 0, lantern: 0, tarp: 0 };

  // total interactions across every hotspot (including night and dog) --
  // a rare scene-wide moment as a reward for spending a while on the page
  const GLOBAL_MILESTONE_EVERY = 30;
  const GLOBAL_MILESTONE_CAPTION = '은은한 새벽빛이 잠시 스쳐 지나갑니다.';
  let totalClickCount = 0;

  function maybeGlobalMilestone() {
    if (totalClickCount % GLOBAL_MILESTONE_EVERY !== 0) return;
    pulseGlow('pulse-dawn', 3200);
    // let the hotspot's own caption show first, then this takes over
    setTimeout(() => showCaption(GLOBAL_MILESTONE_CAPTION, 3600), 500);
  }
  ...
  // every 12th pat: the two regular reactions play back-to-back instead of
  // just one, as a bigger, rarer moment. Doesn't touch dogVariantIndex, so
  // the normal ear/bark alternation carries on unaffected afterward.
  const DOG_MILESTONE_EVERY = 12;
  const DOG_MILESTONE_CAPTION = '쿤이가 신나서 짖다가, 기쁜 듯 꼬리를 흔듭니다!';
  let dogClickCount = 0;

  function playDogMilestone() {
    const ear = DOG_VARIANTS[0];
    const bark = DOG_VARIANTS[1];
    playFxOn(fxDogEl, ear.src, ear.frames, ear.delay);
    const earMs = ear.frames * ear.delay + 150;
    trackTimer(fxDogEl, setTimeout(() => {
  ...
  document.querySelectorAll('.hotspot').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (target === 'dog') {
        dogClickCount += 1;
        const dogCaption = (dogClickCount % DOG_MILESTONE_EVERY === 0)
          ? playDogMilestone()
          : playDogFx();
        showCaption(dogCaption);
        maybeGlobalMilestone();
        return;
      }
